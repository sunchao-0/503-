...
sep=os.path.sep          #当前系统分隔符  os.sep可以取代操作系统特定的路径分隔符。windows下为 “\”
...
def GetFile():
    try:
        ...
        Files =  sorted(os.listdir(path))  #os.listdir(path)返回指定目录下的所有文件和目录名。
        dir_=[]
        file_=[]
        fileQuantity = len(Files)
        for i in Files:
            try:
                i=os.path.join(path, i) #os.path.join(path,name):连接目录与文件名或目录
                if not os.path.isdir(i): #os.path.isfile()和os.path.isdir()函数分别检验给出的路径是一个文件还是目录。'
                    if os.path.islink(i):
...
def DownFile():
    ...
    if os.path.isdir(fileName): #如果是文件夹，先进行压缩在下载
